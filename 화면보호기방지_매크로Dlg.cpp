...
// 화면보호기방지_매크로Dlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "화면보호기방지_매크로.h"
#include "화면보호기방지_매크로Dlg.h"
...
C화면보호기방지매크로Dlg::C화면보호기방지매크로Dlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD___DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void C화면보호기방지매크로Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(C화면보호기방지매크로Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
// C화면보호기방지매크로Dlg 메시지 처리기

BOOL C화면보호기방지매크로Dlg::OnInitDialog()
{
...
void C화면보호기방지매크로Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//  이 함수를 호출합니다.
HCURSOR C화면보호기방지매크로Dlg::OnQueryDragIcon()
{
